...
def transmit_signal(channel_id,transmission_id, msgdata):
    channel_id = channel_id & 0xff
    transmission_id = transmission_id & 0xff
    header = ustruct.pack('<BB', channel_id, transmission_id)
    data = header + msgdata 
    #       2     +  max 29 bytes  => max 31 bytes in adv_data
...
while True:
    ...
    transmission_id = count
    
    # send count as unicode encode string
    msg = str(count)
    # maximal size is 29 bytes in size
    data=msg.encode()[:29]
    
    transmit_signal(channel_id,transmission_id, data)
    count += 1
    count = count%10
